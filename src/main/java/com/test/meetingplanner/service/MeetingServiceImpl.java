...
public class MeetingServiceImpl implements MeetingService {
    ...
    public List<SalleReunion> BlocIntialisation(){
        ...
        return salleReunionSet;
    }

    @Override
    public Set<SalleReunion> ChercherSalleReunion(Reservation reservation) {
        Set<SalleReunion> salleReunions = new HashSet<>();
        salleReunions.addAll(this.BlocIntialisation());
        return salleReunions.stream().filter(salleReunion -> !salleReunion.getReservationSet().contains(reservation)).collect(Collectors.toSet());
    }

    @Override
    public Reservation addReservation(Reservation reservation, int salleId){
        SalleReunion salleReunion = BlocIntialisation().stream().filter(salleReunion1 -> salleReunion1.getId().equals(salleId)).collect(Collectors.toList()).get(0);
        salleReunion.getReservationSet().stream().collect(Collectors.toList()).add(reservation);
        return reservation;
    }

    public List<SalleReunion> findAllSalleReunion(){
        return this.BlocIntialisation();
    }
